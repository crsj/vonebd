...
public class VOneHelper {
    ...
    public VOneHelper() throws Exception {
    	
    	_context = new EnvironmentContext();
    	
    	System.out.println("System.getProperty(\"btlan\") got : " + System.getProperty("btlan"));
    	
    	if (System.getProperty("btlan") !=null && System.getProperty("btlan").trim().length() > 0) {
    		
    		System.out.println("throuth proxy");
    		_metaModel = _context.getMetaModelWithProxy();
    		_services = _context.getServicesWithProxy(); 
    	}
    	else {
    ...
    public Asset[] listOfSprints() throws Exception {
    	
    	 IAssetType timebox = _metaModel.getAssetType("Timebox");
         Query query = new Query(timebox);
    	
         FilterTerm f = new FilterTerm(timebox
         		.getAttributeDefinition("BeginDate"));
         
         f.greater("2013-08-31T12:00:00");
         
         query.setFilter(f);
    	
       
        IAttributeDefinition nameAttribute = timebox.getAttributeDefinition("Name");
        IAttributeDefinition beginDate = timebox.getAttributeDefinition("BeginDate");
        IAttributeDefinition endDate = timebox.getAttributeDefinition("EndDate");
        IAttributeDefinition status = timebox.getAttributeDefinition("State.Code");
    ...
    public Asset[] OIDListOfAssets(Date d, String timeBoxId) throws Exception {
    	
//    	Oid oid = Oid.fromToken("Story", _metaModelWithProxy);
        ...
        //select=Name,Children.ToDo.@Sum&where=Timebox%3D'Timebox:1877'&asof=2013-09-16
       
        query.setAsOf(d);
        FilterTerm f = new FilterTerm(storyType
        		.getAttributeDefinition("Timebox"));
        
        f.equal("Timebox:" + timeBoxId);
        
        query.setFilter(f);
        
        query.getSelection().add( storyType.getAttributeDefinition("Name"));
        query.getSelection().add( storyType.getAttributeDefinition("Children.ToDo.@Sum"));
//        query.getSelection().add( storyType.getAttributeDefinition("AssetState"));
        
//        query.getSelection().add( storyType.getAttributeDefinition("Owners.Name"));
//        query.getSelection().add( storyType.getAttributeDefinition("Estimate"));
//        query.getSelection().add(changedBy);
//        query.getSelection().add(estimateAttribute);
//        query.getSelection().add(retireDate);
    ...
    public Asset[] GetTasksOfTimebox() throws Exception {
    	
//    	Oid oid = Oid.fromToken("Story", _metaModelWithProxy);
    	
    	
        IAssetType storyType = _metaModel.getAssetType("Task");
        Query query = new Query(storyType);
        
        //select=Name,Children.ToDo.@Sum&where=Timebox%3D'Timebox:1877'&asof=2013-09-16
       
        query.setAsOf(new Date());
        FilterTerm f = new FilterTerm(storyType
        		.getAttributeDefinition("Timebox"));
        
        f.equal("Timebox:2311");
        
        query.setFilter(f);
        
        query.getSelection().add( storyType.getAttributeDefinition("Parent.Name"));
        query.getSelection().add( storyType.getAttributeDefinition("Name"));
        query.getSelection().add( storyType.getAttributeDefinition("DetailEstimate"));
        query.getSelection().add( storyType.getAttributeDefinition("ToDo"));
//        query.getSelection().add(estimateAttribute);
//        query.getSelection().add(retireDate);
    ...
    public Asset[] FindListOfAssets() throws Exception
    {
        ...
        IAttributeDefinition children = requestType.getAttributeDefinition("Children");

        query.getSelection().add(nameAttribute);
        query.getSelection().add(children);
        ...
        for (Asset request : result.getAssets())
        {
            ...
            System.out.println(request.getAttribute(children).getValue());
            System.out.println();
        }
        /***** OUTPUT *****
         Request:1195
    ...
    public Asset createStoryThroughProxy(String storyName, String projectToken) throws APIException, URISyntaxException, ConnectionException, OidException {
        IAttributeDefinition nameDef = _services.getAttributeDefinition("Story.Name");
        IAttributeDefinition scopeDef = _metaModel.getAttributeDefinition("Story.Scope");

        final Asset asset = new Asset(_services.getAssetType("Story"));
        asset.setAttributeValue(nameDef, storyName);
        asset.setAttributeValue(scopeDef, _services.getOid(projectToken));
        _services.save(asset);
        return asset;
    }
